<template>
  <div class="equipment-detail-popup" v-show="visible && isEntityVisible" :style="dialogStyle">
...
onMounted(async () => {
  ...
  // 加载字典
  try {
    const [qlDict, jczbDict] = await Promise.all([
      getCachedDictionary("jcsblx_ql"),
      getCachedDictionary("jczbzd"),
    ]);
    const sblxList = qlDict || [];
    sblxDictMap.value = sblxList.reduce((acc: Record<string, string>, cur: any) => {
