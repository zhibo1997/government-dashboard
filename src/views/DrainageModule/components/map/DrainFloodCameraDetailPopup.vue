...
const removeExistingMarkers = () => {
  try {
    if (!viewer.value) return;
    const existing = viewer.value.entities.getById("drain-flood-camera-marker");
...
const startPositionTracking = () => {
  ...
  removePostRender = viewer.value.scene.postRender.addEventListener(() => {
    const entity = viewer.value!.entities.getById("drain-flood-camera-marker");
